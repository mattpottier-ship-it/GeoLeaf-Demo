 ...
 *
 * @param map    - IMapAdapter instance (engine-agnostic).
 * @param config - Configuration object (requires `ui.enableGeolocation`).
 * @returns A destroy function that removes the control and all listeners,
 *          or undefined if initialisation was skipped.
 */
 ...
 * Custom map control for adding new points of interest.
 *
 * @module ui/control-poi-add
 */
function _handlePoiAddAction(e, link, map, config) {
    e.preventDefault();
    ...
    const openForm = (latlng) => {
        ...
        if (!POIAddFormContract.isAddFormAvailable()) {
            if (Log)
                Log.error("[UI.Controls] AddForm.openAddForm not available");
            return;
        }
    ...
    if (userPosition && defaultPosition === "geolocation") {
        if (Log)
            Log.debug("[UI.Controls] Using GPS position for POI add");
        openForm(userPosition);
    }
    else {
        if (Log)
            Log.debug("[UI.Controls] Activating placement mode for POI add");
        POIAddFormContract.activatePlacementMode(map, (result) => {
            if (result?.latlng) {
                ...
                link.classList.remove("disabled");
                if (Log)
                    Log.warn("[UI.Controls] Mode placement cancelled");
            }
        });
    }
}
 ...
 * @param config - Profile configuration including `ui.showAddPoi` and `poiAddConfig.*`.
 * @returns A destroy function to remove the control, or `undefined` if skipped.
 */
function initPoiAddControl(map, config) {
    ...
    if (!POIAddFormContract.isAddFormAvailable() ||
        ...
        Log?.debug("[UI.Controls] POI add control skipped — add-poi plugin not loaded");
        return;
    }
    // Build the container element
    const container = domCreate("div", "geoleaf-ctrl-poi-add geoleaf-ctrl-group geoleaf-ctrl");
    const link = domCreate("a", "", container);
    link.href = "#";
    link.title = getLabel("aria.poi_add.title");
    link.setAttribute("role", "button");
    link.setAttribute("aria-label", getLabel("aria.poi_add.label"));
    // SAFE: SVG static hardcoded, no user data
    const poiSvg = DOMSecurity.createSVGIcon(18, 18, "M12 2 C 6.5 2 2 6.5 2 12 C 2 17.5 6.5 22 12 22 C 17.5 22 22 17.5 22 12 C 22 6.5 17.5 2 12 2 M12 8 L12 16 M8 12 L16 12", { stroke: "currentColor", strokeWidth: "2", fill: "none" });
    link.appendChild(poiSvg);
    // Block map event propagation on this control
    const cleanups = [];
    blockMapPropagation(container, cleanups);
    // Store bound handlers for proper cleanup with removeEventListener
    const handleAddPoi = (e) => _handlePoiAddAction(e, link, map, config);
    const handleKeydown = (e) => {
        if (e.key === "Enter" || e.key === " " || e.key === "Spacebar")
            handleAddPoi(e);
    };
    link.addEventListener("click", handleAddPoi);
    link.addEventListener("keydown", handleKeydown);
    // Add to map via adapter — returns a handle with remove()
    const controlHandle = map.addControl(container, "topleft");
    // Store cleanup on container for external access (e.g. integration tests)
    container._glCleanup = () => {
        link.removeEventListener("click", handleAddPoi);
...
function _getSearchFieldsFromProfile() {
    try {
        ...
        if (!activeProfile.search)
            return null;
        if (!activeProfile.search.filters)
            return null;
...
function _handlePoiAdd() {
    const poiAddBtn = document.querySelector(".geoleaf-ctrl-poi-add a");
    if (poiAddBtn)
        poiAddBtn.click();
}
...
// DOM Builders
function buildTabsDom(panel, titles) {
...
function buildPanelDom(glMain, titles) {
    const panel = document.createElement("div");
    panel.id = PANEL_ID;
...
_app$2.checkPlugins = function (cfg) {
    ...
    if (cfg && cfg.ui && cfg.ui.showAddPoi === true) {
        if (!GeoLeaf$2.POI || !GeoLeaf$2.POI.AddForm) {
            AppLog.warn("⚠️ Config has showAddPoi=true but AddPOI plugin is not loaded. " +
...
async function loadModule(moduleName) {
    switch (moduleName) {
        ...
        case "poi":
            await import('./chunks/geoleaf-poi-core-CqilT8A-.js');
            await Promise.all([
                import('./chunks/geoleaf-poi-renderers-CcnBztPB.js'),
                import('./chunks/geoleaf-poi-extras-CqilT8A-.js'),
            ]);
            break;
        ...
        case "poiCore":
            ...
            break;
        case "poiRenderers":
            await import('./chunks/geoleaf-poi-renderers-CcnBztPB.js');
            break;
        case "poiExtras":
            await import('./chunks/geoleaf-poi-extras-CqilT8A-.js');
            break;
        case "basemapSelector":
            await import('./chunks/geoleaf-basemap-selector-CcnBztPB.js');
            break;
        case "route":
            await import('./chunks/geoleaf-route-CcnBztPB.js');
            break;
        case "layerManager":
            await import('./chunks/geoleaf-layer-manager-DYj3dsML.js');
            break;
        case "legend":
            await import('./chunks/geoleaf-legend-qkSfaX6m.js');
            break;
        case "labels":
            await import('./chunks/geoleaf-labels-BujICUDo.js');
            break;
        case "themes":
            await import('./chunks/geoleaf-themes-co-pW05r.js');
            break;
        case "table":
            await import('./chunks/geoleaf-table-Cyo8rJU4.js');
            break;
        case "search":
            await import('./chunks/geoleaf-search-BtfHMPj5.js').then(function (n) { return n.s; });
            break;
        default: {
...
async function loadAllSecondaryModules() {
    ...
    await Promise.all([
        import('./chunks/geoleaf-poi-renderers-CcnBztPB.js'),
        import('./chunks/geoleaf-poi-extras-CqilT8A-.js'),
        import('./chunks/geoleaf-route-CcnBztPB.js'),
        import('./chunks/geoleaf-layer-manager-DYj3dsML.js'),
        import('./chunks/geoleaf-legend-qkSfaX6m.js'),
        import('./chunks/geoleaf-labels-BujICUDo.js'),
        import('./chunks/geoleaf-themes-co-pW05r.js'),
        import('./chunks/geoleaf-table-Cyo8rJU4.js'),
    ]);
}
