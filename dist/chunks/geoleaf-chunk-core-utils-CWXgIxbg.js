 ...
 * @internal
 */
function _validateProfileStructure(profile, profileId) {
    if (!profile || typeof profile !== "object" || Array.isArray(profile)) {
        throw new Error(`[GeoLeaf] Profile "${profileId}" must be a JSON object — got ${Array.isArray(profile) ? "array" : typeof profile}`);
    }
    const p = profile;
    const errors = [];
    ...
    if (p["geocodingConfig"] !== undefined) {
        const geo = p["geocodingConfig"];
        if (geo["enabled"] !== undefined && typeof geo["enabled"] !== "boolean") {
            errors.push('"geocodingConfig.enabled" must be a boolean');
        }
        if (geo["provider"] !== undefined && typeof geo["provider"] !== "string") {
            errors.push('"geocodingConfig.provider" must be a string');
        }
    }
    if (errors.length > 0) {
        throw new Error(`[GeoLeaf] Profile "${profileId}" validation failed:\n` +
            errors.map((e) => `  ✗ ${e}`).join("\n"));
    }
}
...
function _fetchAndResolveProfile(Loader, baseUrl, timestamp, fetchOptions, isPoiMappingEnabled, self) {
    ...
    return Loader.fetchJson(`${baseUrl}/profile.json?t=${timestamp}`, fetchOptions)
        ...
        return _resolveProfileStep1(profile, isPoiMappingEnabled, Loader, baseUrl, timestamp, fetchOptions, self);
    })
...
/** Default paint for unclustered POI circles. Reacts to `_syncStatus` for pending badge. */
const DEFAULT_UNCLUSTERED_PAINT = {
    "circle-color": ["coalesce", ["get", "colorFill"], "#4a90e5"],
    "circle-radius": ["coalesce", ["get", "radius"], 6],
    "circle-stroke-width": ["case", ["==", ["get", "_syncStatus"], "pending"], 2.5, 1.5],
    "circle-stroke-color": [
        "case",
        ["==", ["get", "_syncStatus"], "pending"],
        "#ff9800",
...
function poisToFeatureCollection(pois, extractCoords, resolveDisplay) {
    ...
    for (const poi of pois) {
        ...
        // Propagate sync status for the pending badge (set by plugin-addpoi)
        if (poi._syncStatus)
            properties._syncStatus = poi._syncStatus;
        features.push({
            type: "Feature",
            geometry: {
                type: "Point",
...
/** Creates and registers a new adapter for `id`. Theme/legend bind to the first map only. */
function _createInstance(id, options) {
...
function init(options = {}) {
    ...
    try {
        ...
        Log.error(`${context} init failed for "${id}":`, err?.message ?? err);
        _notifyError(err);
...
function getMap(mapId) {
    if (mapId)
        return _instances.get(mapId) ?? null;
...
//# sourceMappingURL=geoleaf-chunk-core-utils-CWXgIxbg.js.map
