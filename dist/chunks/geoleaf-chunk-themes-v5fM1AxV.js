import { x as Config, g as getLog, L as Log, p as getLabel, N as FetchHelper, G as DOMSecurity, o as domCreate, I as createElement, C as Core, M as StyleLoader } from './geoleaf-chunk-core-utils-CWXgIxbg.js';
import { G as GeoJSONCore, a as GeoJSONShared, L as Loader, V as VisibilityManager, b as LayerManager$1 } from './geoleaf-chunk-geojson-nlv0TCVM.js';
import { a as POIShared } from './geoleaf-chunk-poi-BFL85CzE.js';
import { L as LayerManager, a as LegendContract, S as StyleSelector } from './geoleaf-chunk-layers-Bl4JW9rv.js';
import { R as Route } from './geoleaf-chunk-route-BYak9B7z.js';
import { L as Legend } from './geoleaf-chunk-legend-BOgQISfA.js';
import { L as Labels, a as LabelButtonManager } from './geoleaf-chunk-labels-D-oB63HP.js';
...
async function _loadLayersInBatches(visibleLayers, batchSize, applyFn) {
    ...
    const _perf = typeof window !== "undefined" && window.__GEOLEAF_PERF__ ? true : false;
    if (_perf) {
    ...
    for (let i = 0; i < visibleLayers.length; i += batchSize) {
        ...
        if (_perf) {
            getLog().info(`[Perf][batch] #${i / batchSize} taille=${batch.length} dur=${(performance.now() - _tBatch).toFixed(1)}ms`);
        }
        if (i === 0) {
...
_ThemeApplier._applyLayerConfig = function (layerConfig) {
    ...
    const _perf = typeof window !== "undefined" && window.__GEOLEAF_PERF__;
    const _t0 = _perf ? performance.now() : 0;
    // Si the layer n'existe pas, essayer de la load automaticment
    let _result;
    if (!layerData) {
        _result = _ThemeApplier._loadLayerFromProfile(layerId).then((loadedLayer) => {
            if (loadedLayer) {
        ...
        _result = _ThemeApplier._setLayerVisibilityAndStyle(layerId, visible, styleId);
    }
    if (_perf) {
        return _result.finally(() => {
            Log.info(`[Perf][applyLayerConfig] ${layerId}: total=${(performance.now() - _t0).toFixed(1)}ms existed=${!!layerData}`);
        });
    }
    return _result;
};
...
//# sourceMappingURL=geoleaf-chunk-themes-v5fM1AxV.js.map
