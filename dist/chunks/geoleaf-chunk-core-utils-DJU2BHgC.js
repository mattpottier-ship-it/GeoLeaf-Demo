...
function padBounds(b, ratio) {
    ...
    return {
        north: b.north + latSpan,
        south: b.south - latSpan,
        east: b.east + lngSpan,
        west: b.west - lngSpan,
    };
}
...
function _buildCaseExpr(key, ruleEntries, defaultVal) {
    ...
    for (const entry of ruleEntries) {
        ...
        if (v !== undefined)
            caseExpr.push(entry.expression, v);
    }
    caseExpr.push(defaultVal ?? _getPaintDefault(key));
    return caseExpr;
}
/**
 ...
 * @param geometryType - Target geometry type ('fill' | 'line' | 'circle')
...
function createClusteredSource(map, id, options) {
...
class MaplibreAdapter {
    ...
    fitBounds(bounds, options) {
        ...
        const mlOpts = {};
        if (options?.padding) {
            // Map GeoLeafPoint {x, y} to MapLibre uniform padding object.
            mlOpts.padding = {
    ...
    addGeoJSONLayer(id, data, options) {
        ...
        const createdSubLayers = addSubLayers(this._map, id, sourceId, geomTypes, flat, layoutBase, effectiveBeforeId, { styleRules, minZoom, maxZoom, showIconsOnMap: showIconsOnMap || undefined });
    ...
    /**
     * Applies a style to an existing layer by updating paint properties
     * on each sub-layer.
     */
        ...
        for (const subType of entry.subLayerTypes) {
            ...
            switch (subType) {
                case "fill": {
                    ...
                    if (hatch?.enabled) {
                        const patternId = buildHatchPatternId(id, hatch);
                        registerHatchPattern(this._map, patternId, hatch);
                    }
                    paint = toFillPaint(flat, id);
                    break;
                }
                case "line":
                case "casing":
                    paint = toLinePaint(flat);
                    break;
                case "circle":
                    paint = toCirclePaint(flat);
                    break;
                default:
                    continue;
            }
            for (const [prop, value] of Object.entries(paint)) {
                this._map.setPaintProperty(subId, prop, value);
            }
        }
    }
    // ── Layer filtering ──────────────────────────────────────────────────────
    ...
    createClusterGroup(id, options) {
        this._assertReady();
        if (this._clusterIds.has(id)) {
            throw new Error(`MaplibreAdapter: cluster group "${id}" already exists.`);
        }
        const ids = createClusteredSource(this._map, id, {
            clusterRadius: options?.clusterRadius ?? 50,
            clusterMaxZoom: options?.clusterMaxZoom ?? 14,
        });
...
function _getText(el, fallback) {
    return el?.textContent ?? fallback;
}
function _processExtChild(child, ext, tags) {
    if (child.nodeType !== 1)
        return;
    const el = child;
    const tagName = el.tagName;
    const localName = el.localName;
    const isGeoLeaf = tagName.toLowerCase().startsWith("geoleaf:") || !!el.namespaceURI?.includes("geoleaf");
...
function _parseGpxExtensions(el) {
    const ext = {};
...
function _processGpxWaypoints(gpxDoc, features) {
    ...
    for (let i = 0; i < waypoints.length; i++) {
        ...
        if (!isNaN(lat) && !isNaN(lon)) {
            ...
            const id = ext.id || _getText(nameElem, `wpt-${i}`);
            features.push({
                type: "Feature",
                id,
...
function _processGpxTracks(gpxDoc, features) {
    ...
    convertGpxToGeoJSON(gpxString) {
        ...
        try {
            ...
            if (parseErrors.length > 0) {
                Log.error("[DataConverter.convertGpxToGeoJSON] XML parsing error:", parseErrors[0].textContent);
                return emptyFC();
            }
            const features = [];
...
function getLabel(key, ...args) {
    if (!_initialized)
        initI18n();
    let label = _overrides[key] ?? _active[key] ?? lang_fr[key] ?? key;
...
export { MapHelpers as $, formatNumber as A, AbstractRenderer as B, Core as C, DOMSecurity as D, createElement$1 as E, poisToFeatureCollection as F, blockMapPropagation as G, events as H, StyleLoader as I, FetchHelper as J, getNestedValue as K, Log as L, Errors as M, CSRFToken as N, AnimationHelper as O, getAnimationHelper as P, ErrorLogger as Q, EventHelpers as R, SENTINEL_POI as S, TimerManager as T, Utils as U, EventListenerManager as V, globalEventManager as W, bus as X, createEventBus as Y, FetchError as Z, FileValidator as _, toCasingPaint as a, PerformanceProfiler as a0, getPerformanceProfiler as a1, LazyLoader as a2, getLazyLoader as a3, setNestedValue as a4, hasNestedPath as a5, clearScaleCache as a6, haversineDistance as a7, resolvePoiColors as a8, StyleValidator as a9, StyleValidatorRules as aa, SimpleTextRenderer as ab, DataNormalizer as ac, DataConverter as ad, ProfileLoader$1 as ae, ConfigNormalizer as af, ProfileLoader as ag, ProfileManager as ah, StorageHelper as ai, TaxonomyManager as aj, debounce as ak, getDistance as al, ensureMap as am, validateCoordinates$1 as an, validateNumber as ao, Helpers as ap, Validators as aq, initI18n as ar, padBounds as as, GeoLeafError as at, toLinePaint as b, collectHatchPatterns as c, toCirclePaint as d, toSourceId as e, toSubLayerId as f, getLog as g, dispatchGeoLeafEvent as h, calculateMapScale as i, isScaleInRange as j, escapeHtml as k, domCreate as l, getLabel as m, normalizeToFlat as n, resolveField as o, Security as p, Config as q, registerHatchPattern as r, styleRulesToPaint as s, toFillPaint as t, CONSTANTS as u, StyleResolver as v, getActiveProfile as w, compareByOrder as x, getColorsFromLayerStyle as y, validateUrl$2 as z };
//# sourceMappingURL=geoleaf-chunk-core-utils-DJU2BHgC.js.map
