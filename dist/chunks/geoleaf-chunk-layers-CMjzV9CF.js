import { L as Log, D as DOMSecurity, l as domCreate, H as events, T as TimerManager, E as createElement, m as getLabel, q as Config, I as StyleLoader, G as blockMapPropagation } from './geoleaf-chunk-core-utils-DJU2BHgC.js';
import { G as GeoJSONCore, a as GeoJSONShared } from './geoleaf-chunk-geojson-B7dIVLdC.js';
import { L as Labels, a as LabelButtonManager, _ as _UIComponents } from './geoleaf-chunk-labels-efCT-cAK.js';
import { L as Legend } from './geoleaf-chunk-legend-DV7bJdIH.js';
import { _ as _ThemeApplier } from './geoleaf-chunk-themes-CywmR8oP.js';
 ...
 */
/**
 * Default basemap definitions (100% usable without API key).
 * Each entry includes both a legacy `url` field (`{s}` subdomain template)
 * and a `tiles` array (MapLibre-compatible, no subdomain placeholder).
 */
const DEFAULT_BASELAYERS = {
    ...
    satellite: {
        label: "Satellite",
        // Esri has no subdomains — single URL works directly
...
// ─── Vector basemap builder ──────────────────────────────────────────────────
/**
 * Applies a vector basemap by calling `map.setStyle(styleUrl)`.
 *
 * NOTE — setStyle() replaces the entire style, clearing all sources and layers.
 * At Sprint 3 this is safe because no data layers exist in MapLibre yet.
 * SPRINT 4 NOTE: GeoJSON / MVT data layers must be re-injected inside the
 * `style.load` callback after every setStyle() call.
 */
function _applyVectorBasemap(nativeMap, definition, key) {
    nativeMap.setStyle(definition.style);
 ...
 * No-op for vector basemaps — setStyle() handles full replacement.
 */
...
function registerBaseLayer(key, definition) {
    ...
    // Validate: must have url, tiles, or style
    const hasRaster = !!definition.url || (Array.isArray(definition.tiles) && definition.tiles.length > 0);
    const hasVector = !!(definition.style || definition.type === "maplibre");
    if (!hasRaster && !hasVector) {
        Log.warn("[GeoLeaf.Baselayers] Invalid definition for layer:", actualKey, "(no url / tiles / style provided)");
        return;
    }
 ...
 *
 * For raster basemaps: injects a MapLibre raster source + layer.
 * For vector basemaps: calls map.setStyle(styleUrl).
 *
...
function setBaseLayer(key, options = {}) {
    ...
    if (_activeKey === key)
        return;
    // Defer until the map's style is fully loaded
    ...
    const type = _resolveBasemapType(definition);
    _removeCurrentBasemap();
    try {
        if (type === "vector") {
            _applyVectorBasemap(_map, definition, key);
        }
        else {
            _applyRasterBasemap(_map, definition);
        }
    }
    catch (e) {
 ...
 * @param options.map - Leaflet map instance (uses the global map if omitted).
...
class NotificationSystem {
    ...
    _remove(toast, isReorganization = false) {
        ...
        if (toast.dataset.timerId && this._timerManager) {
            this._timerManager.clearTimeout(parseInt(toast.dataset.timerId, 10));
 ...
 */
function _setButtonState(btnId, disabled, text) {
...
function _renderDownloadedStatus(stateEl, sizeEl, clearBtn, totalSize) {
    if (stateEl) {
...
function _renderNotDownloadedStatus(stateEl, sizeEl, clearBtn) {
    if (stateEl) {
...
function _renderCacheStatus(profileId, status, quota) {
    ...
    if (quotaEl) {
        quotaEl.textContent = getLabel("format.cache.quota_mb", (quota.available / 1024 / 1024).toFixed(2));
    }
}
function _buildCacheStatusHtml() {
...
function _buildCacheActionsHtml() {
...
function _notifyDownloadSuccess(result, progressText, progressEl) {
    if (progressText) {
        progressText.textContent = getLabel("ui.cache.progress_done", String(result.resourcesCount));
    }
    _UINotifications.success(getLabel("toast.cache.download_success", (result.totalSize / 1024 / 1024).toFixed(2)), 4000);
    setTimeout(() => {
        if (progressEl)
            progressEl.style.display = "none";
    }, 2000);
}
function _notifyDownloadError(error, progressText, progressEl) {
    if (progressText) {
...
const CacheSection = {
    ...
    async _handleDownload() {
        ...
        try {
            ...
            if (progressText)
                progressText.textContent = getLabel("ui.cache.progress_preparing");
            ...
            _notifyDownloadError(error, progressText, progressEl);
        }
        finally {
    ...
    async _handleClear() {
        if (!window.confirm(getLabel("ui.cache.confirm_clear")))
            return;
        const profileId = Config.get("data.activeProfile", "");
        try {
            await StorageContract.CacheManager.clearCache(profileId);
            _UINotifications.success(getLabel("toast.cache.cleared"), 3000);
            await this._updateStatus();
        }
        catch (error) {
            Log.error(`[CacheSection] Clear failed: ${error.message}`);
            _UINotifications.error(getLabel("toast.cache.clear_error", error.message), 5000);
        }
    },
};
...
function _applyStyleResult(layerData, layerId, styleId, res) {
    ...
    // Style files have the shape { id, label, style: { fillColor, radius, … }, legend }.
    // normalizeToFlat() is a shallow copy — passing the root object would leave
    // paint properties nested inside flat.style and toCirclePaint/toFillPaint would
    // find undefined for fillColor, radius, etc. We unwrap here so the adapter
    // receives a flat paint object directly.
    const paintData = res.styleData?.style ?? res.styleData;
...
const _LayerManagerStyleSelector = {
    ...
    _applyStyleLegacy(layerId, styleId, layerData, styleConfig) {
        ...
        fetch(stylePath)
            ...
            // flat-paint object from the style file JSON root.
            const paintData = styleData?.style ?? styleData;
            if (typeof GeoJSONCore.setLayerStyle === "function") {
                GeoJSONCore.setLayerStyle(layerId, paintData);
            }
            if (LegendContract.isAvailable()) {
...
export { BasemapSelector as B, CacheSection as C, LayerManager as L, NotificationSystem as N, StorageContract as S, _UINotifications as _, LegendContract as a, StyleSelector as b, LMControl as c, LMRenderer as d, LMShared as e, Baselayers as f };
//# sourceMappingURL=geoleaf-chunk-layers-CMjzV9CF.js.map
