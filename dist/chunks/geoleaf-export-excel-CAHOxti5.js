 ...
 * SheetJS (xlsx) is bundled exclusively in this chunk — zero cost for other formats.
 *
 * @module lazy/export-excel
 */
/**
 * Builds an Excel workbook buffer from an array of GeoJSON features.
 * Properties become columns; geometry is excluded.
 */
function buildExcelBuffer(features) {
    ...
    const worksheet = utils.json_to_sheet(rows, { header: keys });
    const workbook = utils.book_new();
    utils.book_append_sheet(workbook, worksheet, "GeoLeaf Export");
    // XLSX.write with type:"array" returns a plain number[], not a Uint8Array.
    return new Uint8Array(writeSync(workbook, { type: "array", bookType: "xlsx" }));
}

export { buildExcelBuffer };
//# sourceMappingURL=geoleaf-export-excel-CAHOxti5.js.map
