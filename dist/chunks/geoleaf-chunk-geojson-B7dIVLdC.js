import { g as getLog, n as normalizeToFlat, c as collectHatchPatterns, r as registerHatchPattern, s as styleRulesToPaint, t as toFillPaint, a as toCasingPaint, b as toLinePaint, d as toCirclePaint, S as SENTINEL_POI, e as toSourceId, f as toSubLayerId, h as dispatchGeoLeafEvent, i as calculateMapScale, j as isScaleInRange, L as Log } from './geoleaf-chunk-core-utils-DJU2BHgC.js';
...
function _bindVtHoverTooltip(nativeMap, subLayerIds, def) {
    ...
    for (const subId of subLayerIds) {
        if (!nativeMap.getLayer(subId))
            continue;
        nativeMap.on("mouseenter", subId, (e) => {
            if (!e.features || e.features.length === 0)
                return;
            const props = e.features[0].properties ?? {};
            ...
            tooltipPopup
                .setLngLat(e.lngLat)
                .setHTML(_escapeHtml$1(String(propValue)))
                .addTo(nativeMap);
        });
        nativeMap.on("mouseleave", subId, () => {
            if (tooltipPopup) {
...
const VectorTiles = {
    ...
    _resolveTileUrl(def, vtConfig) {
        if (vtConfig.url) {
            if (vtConfig.url.startsWith("http") ||
    ...
    async loadVectorTileLayer(layerId, layerLabel, def, _baseOptions) {
        ...
        const styleFlat = styleData?.defaultStyle
            ? normalizeToFlat(styleData.defaultStyle)
            : defaultFlat;
 ...
 */
function _getTooltipText(props) {
...
PopupTooltip.bindUnifiedTooltip = function (feature, layer, def) {
    ...
    if (tooltipMode === "never")
        return;
    const props = feature.properties;
    const tooltipText = _getTooltipText(props);
    const GeoLeaf = _getGeoLeaf();
    const Loader = GeoLeaf && GeoLeaf._GeoJSONLoader;
...
function bindMapLibreTooltip(layerId, def, nativeMap, subLayerIds) {
    ...
    for (const subId of subLayerIds) {
        if (!nativeMap.getLayer(subId))
            continue;
        nativeMap.on("mouseenter", subId, (e) => {
            // Always remove any lingering tooltip before creating a new one.
            // Without this, quick mouse moves across sub-layers orphan the previous
            // popup (reference is overwritten) leaving it stuck on the map forever.
            if (hoverPopup) {
                hoverPopup.remove();
                hoverPopup = null;
            }
            ...
            hoverPopup = new PopupCtor({
                closeButton: false,
                closeOnClick: false,
                className: "gl-geojson-tooltip",
                offset: [0, -10],
            });
            hoverPopup.setLngLat(e.lngLat).setHTML(content).addTo(nativeMap);
        });
        nativeMap.on("mouseleave", subId, () => {
            if (hoverPopup) {
                hoverPopup.remove();
...
function _applyVectorTilesConfig(nd, d) {
    if (d.data &&
        ...
        nd.vectorTiles = d.data.vectorTiles;
...
function _getDataPromise(fromCache, isGpx, useWorker, def, WorkerMgr, layerId) {
    if (fromCache)
...
function _convertRawData(rawData, isGpx, DataConverter) {
    if (isGpx) {
...
const Loader = {};
async function _doLoadSingleLayerMapLibre(rawData, fromCache, isGpx, def, layerId, layerLabel, state, Log) {
    if (fromCache)
    ...
    if (clusterResult.shouldCluster) {
        ...
        adapterOptions.clusterMaxZoom =
            typeof def.disableClusteringAtZoom === "number" ? def.disableClusteringAtZoom : 14;
...
Loader._loadSingleLayer = function (layerId, layerLabel, def, baseOptions) {
    ...
    return dataPromise.then((rawData) => _doLoadSingleLayerMapLibre(rawData, fromCache, isGpx, def, layerId, layerLabel, state, Log));
};
...
//# sourceMappingURL=geoleaf-chunk-geojson-B7dIVLdC.js.map
