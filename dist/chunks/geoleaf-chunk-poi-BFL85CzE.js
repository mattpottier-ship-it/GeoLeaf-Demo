import { q as resolveField$1, L as Log, u as Security, x as Config, y as CONSTANTS, z as StyleResolver, g as getLog, A as getActiveProfile, B as compareByOrder, m as escapeHtml, D as getColorsFromLayerStyle, E as validateUrl, F as formatNumber$1, G as DOMSecurity, i as dispatchGeoLeafEvent, H as AbstractRenderer, p as getLabel, I as createElement, J as poisToFeatureCollection } from './geoleaf-chunk-core-utils-CWXgIxbg.js';
...
// ========================================
/**
 * Deserialize MapLibre-serialized feature properties.
 * MapLibre GL stringifies array/object values when retrieving features via queryRenderedFeatures.
 * This restores them to native JS values so downstream code can work with real arrays/objects.
 */
function _deserializeProperties(props) {
    const result = {};
    for (const [key, val] of Object.entries(props)) {
        if (typeof val === "string" && (val.charCodeAt(0) === 91 /* [ */ || val.charCodeAt(0) === 123 /* { */)) {
            try {
...
/**
 * Coerce a value that may be a real array or a MapLibre-serialized JSON string back to an array.
 * MapLibre GL stringifies array/object feature properties on retrieval.
 */
function _tryParseArray(raw) {
...
function _resolveCategories(attr, p, props) {
    return {
        categoryId: attr.categoryId || p.categoryId || p.category ||
            props.categoryId || props.category || null,
        subCategoryId: attr.subCategoryId || attr.subcategoryId ||
            p.subCategoryId || p.subcategory ||
            props.subCategoryId || props.subcategoryId || props.sub_category || null,
    };
}
...
function _resolveMainImage(attr, p, props) {
    ...
    return _sanitizeUrl(attr.photo || attr.mainImage || attr.photo_principale ||
        props.photo || props.mainImage || props.photo_principale ||
        galleryFirst || null);
}
/** Collect raw gallery array from multiple sources. */
function _rawGallery(attr, p, props) {
    const raw = attr.gallery ?? p.gallery ?? props.gallery ?? p.images ?? props.images;
    return _tryParseArray(raw);
}
...
/** Resolve opening hours from multiple sources. Coerces MapLibre-stringified arrays. */
function _resolveOpeningHours(attr, p, props) {
    const raw = attr.opening_hours ??
        attr.openingHours ??
        p.opening_hours ??
        props.opening_hours ??
        p.openingHours ??
...
function _resolvePoiTags(attr, p, props) {
    const raw = _tryParseArray(attr.tags ?? p.tags ?? props.tags);
    return raw.map((t) => _escapeHtml(String(t)));
}
function _resolvePoiServices(attr, p, props) {
    const raw = _tryParseArray(attr.services ?? p.services ?? props.services);
...
function normalizePoi(poi) {
    ...
    // Deserialize MapLibre-serialized properties so array/object values are native JS.
    const props = _deserializeProperties(p.properties || {});
 ...
 * Pas de HTML complexe, optimisé pour tooltips MapLibre.
 *
 * Process:
 ...
 */
function _normalizeTagsInput(tags) {
    ...
    attachGalleryEvents(sidePanelElement, lightboxManager) {
        ...
        thumbs.forEach((thumb) => {
            thumb.addEventListener("click", () => {
                const index = Number.parseInt(thumb.getAttribute("data-index") ?? "0", 10);
        ...
        mainImg.addEventListener("click", () => {
            if (lightboxManager) {
                ...
                const currentIndex = activeThumb
                    ? Number.parseInt(activeThumb.getAttribute("data-index") ?? "0", 10)
                    : 0;
...
/**
 * Re-creates the POI cluster source after `map.setStyle()` destroys it.
 * Called by the `geoleaf:style:rebuild` event listener.
 *
 * 1. Re-creates the cluster group via the adapter (source + layers).
 * 2. Re-registers sprite icons (cleared by setStyle).
 * 3. Re-binds click events on the unclustered-point layer.
 * 4. Re-populates the source with the current POI data.
 *
 * @internal
 */
async function _rebuildPoiClusterSource(state, constants) {
    const _g = globalThis;
    const adapter = _g.GeoLeaf?.Core?.getAdapter?.();
    if (!adapter || !state.poiSourceId)
        return;
    const nativeMap = adapter.getNativeMap?.();
    if (!nativeMap)
        return;
    try {
        // Recreate cluster source via adapter (async — waits for style if needed)
        await adapter.createClusterGroup(state.poiSourceId, {
        ...
        // Re-register sprite icons (cleared by setStyle)
        registerSpriteIcons(nativeMap).catch((err) => {
            if (Log)
                Log.warn("[POI] Rebuild: failed to re-register sprite icons:", err);
        });
        // Rebind POI click events on the new unclustered-point layer
        bindMapLibrePoiEvents(adapter, nativeMap, state);
        // Re-populate with current POI data
        if (Array.isArray(state.allPois) && state.allPois.length > 0) {
            const fc = _buildPoiFeatureCollection(state.allPois);
            adapter.updateLayerData(state.poiSourceId, fc);
        }
        if (Log)
            Log.info(`[POI] Rebuild: cluster source restored (${state.allPois?.length ?? 0} POIs).`);
    }
    catch (err) {
        if (Log)
            Log.warn("[POI] Rebuild: failed to restore cluster source:", err);
    }
}
async function _initMapLibrePoi(state, constants) {
    ...
    // When map.setStyle() destroys all sources/layers, this re-creates the
    // POI cluster source, re-registers sprite icons, and re-binds events.
    document.addEventListener("geoleaf:style:rebuild", () => {
        _rebuildPoiClusterSource(state, constants).catch((err) => {
            if (Log)
                Log.warn("[POI] Rebuild: unexpected error:", err);
        });
    });
...
async function init(mapOrOptions, config) {
    ...
    // Resolve MapLibre adapter regardless of enabled flag — sprite registration
    // is required for GeoJSON showIconsOnMap layers even when the POI module is disabled.
    const _g = globalThis;
    const adapter = _g.GeoLeaf?.Core?.getAdapter?.();
    const nativeMap = adapter?.getNativeMap?.();
    if (state.poiConfig.enabled === false) {
        // POI markers/clusters are disabled, but we still need to load the profile
        // sprite so that GeoJSON symbol layers (showIconsOnMap: true) can resolve
        // their icon images inside MapLibre's image registry.
        if (nativeMap && typeof nativeMap.addSource === "function") {
            await _initPoiSprite();
            await registerSpriteIcons(nativeMap);
            // Re-register after every basemap style reset (setStyle destroys images).
            document.addEventListener("geoleaf:style:rebuild", () => {
                registerSpriteIcons(nativeMap).catch((err) => {
                    if (Log)
                        Log.warn("[POI] Rebuild: failed to re-register sprite icons:", err);
...
function loadAndMergeStoredPois(existingPois, callback) {
    ...
    if (!checkStorageAvailable()) {
        const _g = globalThis;
        const pluginsApi = _g.GeoLeaf?.plugins;
        if (!pluginsApi?.isLoaded?.("storage")) {
            // Storage plugin not loaded — skip silently, no POIs to merge
            callback(existingPois);
            return;
        }
...
/**
 * Updates the sync status of a POI and re-renders the cluster source.
 * Used by plugin-addpoi to show/hide the pending badge on markers.
 *
 * @param poiId - POI identifier.
 * @param status - `"pending"` to show badge, `null` or `"synced"` to clear it.
 */
function updatePoiSyncStatus(poiId, status) {
    const shared = POIShared;
    if (!shared)
        return;
    const state = shared.state;
    ...
    if (!status || status === "synced") {
        delete poi._syncStatus;
    }
    else {
        poi._syncStatus = status;
    }
    if (state.adapter && state.poiSourceId) {
        const fc = _buildPoiFeatureCollection(state.allPois);
        state.adapter.updateLayerData(state.poiSourceId, fc);
...
/**
 * Renames a POI's ID in state and re-renders (used after server assigns a permanent ID).
 *
 * @param oldId - Temporary POI ID.
 * @param newId - Permanent ID returned by the server.
 */
function updatePoiId(oldId, newId) {
    const shared = POIShared;
    if (!shared)
        return;
    const state = shared.state;
    const poi = state.allPois.find((p) => p.id === oldId);
    if (!poi)
        return;
    poi.id = newId;
    if (state.adapter && state.poiSourceId) {
    ...
    if (Log)
        Log.debug(`[POI] updatePoiId: ${oldId} → ${newId}`);
}
/**
...
const POI = {
    ...
    /**
     * Sets or clears the sync status of a POI, re-rendering the cluster source.
     * Used by plugin-addpoi to show/hide the pending badge (orange stroke) on markers.
     *
     * @param poiId - POI identifier.
     * @param status - `"pending"` to show badge, `null` or `"synced"` to clear it.
     */
    updatePoiSyncStatus: function (poiId, status) {
        if (!POICore)
            return;
        POICore.updatePoiSyncStatus(poiId, status);
    },
    /**
     * Renames a POI's ID after the server assigns a permanent identifier.
     * Called by plugin-addpoi sync handler after a successful add operation.
     *
     * @param oldId - Temporary POI ID (e.g. `user-poi-1234567890`).
     * @param newId - Permanent ID returned by the server.
     */
    updatePoiId: function (oldId, newId) {
        if (!POICore)
            return;
        POICore.updatePoiId(oldId, newId);
    },
    /**
...
const POICoreContract = {
     ...
     */
    addPoi(poi) {
        // Route through the global GeoLeaf.POI.addPoi when available so that the
        // core bundle's initialized POIShared state is used (not the plugin bundle's
        // own inlined copy, which is never initialized).
        const g = (typeof globalThis !== "undefined" ? globalThis : window);
        if (typeof g?.GeoLeaf?.POI?.addPoi === "function") {
            return g.GeoLeaf.POI.addPoi(poi);
        }
        return POICore.addPoi(poi);
    },
    /**
...
//# sourceMappingURL=geoleaf-chunk-poi-BFL85CzE.js.map
