import { L as Log, v as validateUrl, i as dispatchGeoLeafEvent, G as DOMSecurity, o as domCreate, x as Config, M as StyleLoader, p as getLabel, K as blockMapPropagation } from './geoleaf-chunk-core-utils-4nAkV2K5.js';
import { G as GeoJSONCore, a as GeoJSONShared } from './geoleaf-chunk-geojson-DSRPRrfB.js';
import { L as Labels, a as LabelButtonManager, _ as _UIComponents } from './geoleaf-chunk-labels-BTU7yY5G.js';
import { L as Legend } from './geoleaf-chunk-legend-BkwP5jUw.js';
import { _ as _ThemeApplier } from './geoleaf-chunk-themes-Cu5pm4lB.js';
 ...
 *
 * Providers requiring an API key (Thunderforest, Jawg Maps) use `{apikey}`
 * in their tile URLs — resolved at apply time via `definition.apiKey`.
 *
 * IGN entries use the public Géoportail endpoint (data.geopf.fr) which
 * requires no API key for standard layers since 2023.
 */
const DEFAULT_BASELAYERS = {
    ...
    // ── Stadia Maps ────────────────────────────────────────────────────────────
    // No API key required for dev/testing.
    // For production quotas: set apiKey in the basemap definition — injected as
    // ?api_key={apikey} query param by normalizeTilesArray at apply time.
    "stadia-alidade-smooth": {
        label: "Stadia Smooth",
        tiles: ["https://tiles.stadiamaps.com/tiles/alidade_smooth/{z}/{x}/{y}.png?api_key={apikey}"],
        maxZoom: 20,
    ...
    "stadia-stamen-toner": {
        label: "Stadia Toner",
        tiles: ["https://tiles.stadiamaps.com/tiles/stamen_toner/{z}/{x}/{y}.png?api_key={apikey}"],
        maxZoom: 20,
    ...
    "stadia-stamen-terrain": {
        ...
        tiles: ["https://tiles.stadiamaps.com/tiles/stamen_terrain/{z}/{x}/{y}.png?api_key={apikey}"],
        maxZoom: 18,
        attribution: "&copy; Stadia Maps &copy; Stamen Design &copy; OpenMapTiles &copy; OpenStreetMap contributors",
    },
    // ── Thunderforest ──────────────────────────────────────────────────────────
    // API key required (apiKeyRequired: true). Without apiKey the basemap is
    // disabled with a console warning — Thunderforest returns HTTP 403 without a key.
    "thunderforest-landscape": {
        label: "Thunderforest Landscape",
        apiKeyRequired: true,
        tiles: [
            "https://a.tile.thunderforest.com/landscape/{z}/{x}/{y}.png?apikey={apikey}",
            "https://b.tile.thunderforest.com/landscape/{z}/{x}/{y}.png?apikey={apikey}",
            "https://c.tile.thunderforest.com/landscape/{z}/{x}/{y}.png?apikey={apikey}",
        ],
        maxZoom: 22,
    ...
    "thunderforest-transport": {
        ...
        apiKeyRequired: true,
        tiles: [
            "https://a.tile.thunderforest.com/transport/{z}/{x}/{y}.png?apikey={apikey}",
            "https://b.tile.thunderforest.com/transport/{z}/{x}/{y}.png?apikey={apikey}",
            "https://c.tile.thunderforest.com/transport/{z}/{x}/{y}.png?apikey={apikey}",
        ],
        maxZoom: 22,
        attribution: "Maps &copy; <a href='https://www.thunderforest.com'>Thunderforest</a>, Data &copy; OpenStreetMap contributors",
    },
    // ── Jawg Maps ──────────────────────────────────────────────────────────────
    // API key required (apiKeyRequired: true). Jawg returns an error tile without key.
    "jawg-streets": {
        label: "Jawg Streets",
        apiKeyRequired: true,
        tiles: [
            "https://a.tile.jawg.io/jawg-streets/{z}/{x}/{y}.png?access-token={apikey}",
            "https://b.tile.jawg.io/jawg-streets/{z}/{x}/{y}.png?access-token={apikey}",
            "https://c.tile.jawg.io/jawg-streets/{z}/{x}/{y}.png?access-token={apikey}",
            "https://d.tile.jawg.io/jawg-streets/{z}/{x}/{y}.png?access-token={apikey}",
        ],
        maxZoom: 22,
    ...
    "jawg-terrain": {
        ...
        apiKeyRequired: true,
        tiles: [
            "https://a.tile.jawg.io/jawg-terrain/{z}/{x}/{y}.png?access-token={apikey}",
            "https://b.tile.jawg.io/jawg-terrain/{z}/{x}/{y}.png?access-token={apikey}",
            "https://c.tile.jawg.io/jawg-terrain/{z}/{x}/{y}.png?access-token={apikey}",
            "https://d.tile.jawg.io/jawg-terrain/{z}/{x}/{y}.png?access-token={apikey}",
        ],
        maxZoom: 22,
        attribution: "&copy; <a href='https://www.jawg.io'>Jawg Maps</a> &copy; OpenStreetMap contributors",
    },
    "jawg-sunny": {
        ...
        apiKeyRequired: true,
        tiles: [
            "https://a.tile.jawg.io/jawg-sunny/{z}/{x}/{y}.png?access-token={apikey}",
            "https://b.tile.jawg.io/jawg-sunny/{z}/{x}/{y}.png?access-token={apikey}",
            "https://c.tile.jawg.io/jawg-sunny/{z}/{x}/{y}.png?access-token={apikey}",
            "https://d.tile.jawg.io/jawg-sunny/{z}/{x}/{y}.png?access-token={apikey}",
        ],
        maxZoom: 22,
        attribution: "&copy; <a href='https://www.jawg.io'>Jawg Maps</a> &copy; OpenStreetMap contributors",
    },
    // ── IGN Géoportail (WMTS public — no API key required for standard layers) ──
    // Uses data.geopf.fr, the public IGN endpoint since 2023.
    // Optional apiKey injected as &apikey= in getCapabilitiesUrl for high-quota access.
    "ign-plan": {
        label: "IGN Plan",
        type: "wmts",
        wmts: {
...
function _findWmtsLayer(doc, layerName) {
    ...
    if (!layerName)
        return layers[0];
    const match = layers.find((l) => _getText(l, "Identifier") === layerName);
    return match ?? layers[0];
}
/**
...
function _findTileMatrixSetId(layer, tileMatrixSet) {
    ...
    if (!fallbackLinks.length)
        return null;
    if (tileMatrixSet) {
        const match = fallbackLinks.find((l) => _getText(l, "TileMatrixSet") === tileMatrixSet);
        if (match)
            return _getText(match, "TileMatrixSet");
    }
 ...
 * @internal
 */
function _toXyzTemplate(template, tmsId) {
    return template
 ...
 * @internal
 */
function _buildKvpFallback(doc, layer, tmsId, format) {
    if (!tmsId)
        return null;
...
function _extractTileUrl(doc, layer, tmsId, format) {
    ...
    for (const ru of resourceUrls) {
        ...
        if (template)
            return _toXyzTemplate(template, tmsId);
    }
    return _buildKvpFallback(doc, layer, tmsId, format);
}
// ─── WMTS public API ──────────────────────────────────────────────────────────
/**
...
async function resolveWmtsTilesUrl(definition, signal) {
    ...
    const cached = _wmtsCache.get(safeCapsUrl);
    if (cached) {
        Log.debug("[GeoLeaf.WMTS] Cache hit for:", safeCapsUrl);
        return cached;
    }
    ...
    try {
        ...
        _wmtsCache.set(safeCapsUrl, tileUrl);
...
//# sourceMappingURL=geoleaf-chunk-layers-CmsLxCRt.js.map
