import { L as Log, l as domCreate, G as blockMapPropagation, D as DOMSecurity, m as getLabel, q as Config, x as compareByOrder } from './geoleaf-chunk-core-utils-DJU2BHgC.js';
import { P as POIMarkers, a as POIShared } from './geoleaf-chunk-poi-DjAIl4u9.js';
import { _ as _UIComponents } from './geoleaf-chunk-labels-efCT-cAK.js';
        ...
        new Promise((resolve) => {
            const observer = new MutationObserver((_mutations, obs) => {
                ...
                if (el) {
                    ...
                    if (!_spriteDetected) {
                        if (Log)
                            Log.info("[Legend] SVG sprite detected via MutationObserver");
                        _spriteDetected = true;
                    }
                    resolve(true);
                }
            ...
            const timerId = setTimeout(() => {
                observer.disconnect();
                if (Log)
                    Log.warn("[Legend] SVG sprite not found after 2s");
                resolve(false);
            }, 2000);
        }).then((found) => {
            if (typeof callback === "function")
...
function generateLegendFromStyle(styleData, geometryType, taxonomyData) {
    ...
    if (Array.isArray(styleData.styleRules) && styleData.styleRules.length > 0) {
        styleData.styleRules.forEach((rule) => {
            ...
            const item = generateLegendItem(rule.style, rule.legend, geometryType, styleData.style ?? null, rule, taxonomyData, symbolPrefix);
            if (item) {
                items.push(item);
            }
        });
...
//# sourceMappingURL=geoleaf-chunk-legend-DV7bJdIH.js.map
